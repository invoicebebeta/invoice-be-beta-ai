...
import { ensureConnectedAccountsTable } from "./connectDb";
import app from "./app";
...
initStripe()
  ...
  .catch((err) => {
    logger.error({ err }, "Failed to initialize Stripe — starting without it");
    app.listen(port, (err) => {
      if (err) {
        logger.error({ err }, "Error listening on port");
        process.exit(1);
      }
      logger.info({ port }, "Server listening (Stripe unavailable)");
    });
  });
