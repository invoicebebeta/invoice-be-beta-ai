...
export function buildInvoiceHtml(
  invoice: Invoice,
  user: { businessName?: string; email?: string; logoUri?: string; invoiceColor?: string; vatNumber?: string; businessAddress?: string; bankDetails?: { accountHolderName: string; sortCode: string; accountNumber: string; bankName?: string; reference?: string } } | null
): string {
...
<body>
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:40px;">
    <div style="display:flex;align-items:center;gap:14px;">
      ...
      <div>
        ...
        ${user?.businessAddress ? `<div style="font-size:12px;color:#374151;margin-top:2px;">${user.businessAddress.replace(/\n/g, ' · ')}</div>` : ""}
        ${user?.vatNumber ? `<div style="font-size:12px;color:#374151;margin-top:2px;">VAT: ${user.vatNumber}</div>` : ""}
      </div>
    </div>
  ...
  <div style="margin-top:48px;border-top:1px solid #e5e7eb;padding-top:16px;text-align:center;font-size:11px;color:#9ca3af;">
    ${[user?.businessAddress?.replace(/\n/g, ' · '), user?.vatNumber ? `VAT: ${user.vatNumber}` : ''].filter(Boolean).join(' &middot; ')}
    ${user?.businessAddress || user?.vatNumber ? '<br>' : ''}
    Generated by Invoice Be AI
  </div>
</body>
</html>`;
}
